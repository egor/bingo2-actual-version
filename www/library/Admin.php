...
{
return false;
  if (($row = $this->db->fetchAll("SELECT * FROM catalog_ WHERE type='section'"))) {
    foreach ($row as $res) {
      $this->db->query("UPDATE catalog set pic='$res[pic]', pic_alt='$res[pic_alt]', pic_title='$res[pic_title]', header='$res[header]', title='$res[title]', keywords='$res[keywords]', description='$res[description]' WHERE href='$res[href]'");
    }
